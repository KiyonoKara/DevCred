...
export const loginUser = (
  ...
  cy.contains("Welcome to DevCred!");
...
export const seedDatabase = () => {
  cy.exec(
    "npx ts-node ../server/seedData/populateDB.ts " + Cypress.env("MONGODB_URI")
  );
};
...
export const verifyDMConversationCount = (expectedCount: number) => {
  cy.get(".chats-list h3").should(
    "contain",
    `Your Conversations (${expectedCount})`
  );
};
